...
export class DocumentProcessorInjector {
  ...
  _executeRowLoopOperation(documentId, op) {
    try {
      this.facade.logger.info(`Executing row loop: ${op.dataArray.length} items at table ${op.tableIndex}, row ${op.rowIndex}`);
      const templateRow = this.facade.documentService.getTableRow(documentId, op.tableIndex, op.rowIndex);
      if (!templateRow || !templateRow.cells) {
        this.facade.logger.warn(`Could not get template row data for table ${op.tableIndex}, row ${op.rowIndex}`);
        return;
      }
      ...
      for (let i = op.dataArray.length - 1; i >= 0; i--) {
        ...
        const renderedCells = cellTemplates.map((template) => {
          if (template && template.includes('{{')) return this.facade.mustache.render(template, dataItem);
          return template || '';
        });
        this.facade.documentService.insertTableRow(documentId, op.tableIndex, op.rowIndex + 1, renderedCells);
      }
  ...
  _executeColumnLoopOperation(documentId, op) {
    try {
      this.facade.logger.info(`Executing column loop: ${op.dataArray.length} items at table ${op.tableIndex}, column ${op.cellIndex}`);
      ...
      if (op.dataArray.length > 1) {
        for (let i = 1; i < op.dataArray.length; i++) {
          ...
          this.facade.documentService.insertTableColumn(documentId, op.tableIndex, op.cellIndex + i, cellValues);
        }
        this.facade.logger.debug(`Inserted ${op.dataArray.length - 1} additional columns`);
      }
      this.facade.logger.debug(`Column loop completed for column ${op.cellIndex}: ${op.dataArray.length} columns total`);
    } catch (error) {
  ...
  _convertOperationToRequests(op) {
    switch (op.type) {
      case 'textSubstitution': return this.facade._createTextSubstitutionRequests(op);
      case 'rowLoop': return []; // Handled via Standard API
      case 'deleteRow': return this.facade._createDeleteRowRequests(op);
      case 'listLoop': return this.facade._createListLoopRequests(op);
      case 'columnLoop': return []; // Handled via Standard API
      default:
  ...
  _createTextSubstitutionRequests(op) {
    let originalText = op.originalText, newText = op.newText;
    ...
    if (newText === '') newText = '\u200B';
    return [
      { deleteContentRange: { range: { startIndex: op.index, endIndex: op.index + originalText.length } } },
      { insertText: { location: { index: op.index }, text: newText } }
    ];
  }

  _createDeleteRowRequests(op) {
    return [{ deleteTableRow: { tableCellLocation: { tableStartLocation: { index: op.tableIndex }, rowIndex: op.rowIndex, columnIndex: 0 } } }];
  }

  _createListLoopRequests(op) {
    const requests = [];
    requests.push({ deleteContentRange: { range: { startIndex: op.index, endIndex: op.index + op.fullMatch.length } } });
    for (let i = op.dataArray.length - 1; i >= 0; i--) {
      ...
      requests.push({ insertText: { location: { index: op.index }, text: isLastItem ? itemText : itemText + '\n' } });
      if (op.listType === 'bullet') {
        requests.push({ createParagraphBullets: { range: { startIndex: op.index, endIndex: op.index + itemText.length + 1 }, bulletPreset: 'BULLET_DISC_CIRCLE_SQUARE' } });
      }
    }
    return requests;
  }
