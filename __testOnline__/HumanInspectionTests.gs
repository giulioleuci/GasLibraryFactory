...
function initHumanInspectionTests() {
  runner.register('HumanInspection/Full_Suite', () => {
    ...
    Logger.log(
      '[HumanInspection] TEMPLATE Doc    : ' + DriveApp.getFileById(templateDocId).getUrl()
    );
    ...
    templateSheet
      .getRange('A3')
      .setValue(
        '{{matrice_dati[sorgente=sellers, colonne=name;region;target;achieved;rate, intestazioni=Name;Region;Target;Achieved;%Rate]}}'
      );
    ...
    templateSheet
      .getRange('B12')
      .setValue('{{dynamic_columns[source=months, value=label, acl=ownerEmail, scope=column]}}');

    SpreadsheetApp.flush();
    const templateSSId = templateSS.getId();
    Logger.log(
      '[HumanInspection] TEMPLATE Sheet  : ' + DriveApp.getFileById(templateSSId).getUrl()
    );
    ...
    SmartAssert.isFalse(
      flat.some((c) => c.includes('{{')),
      'Sheet: no unreplaced placeholders'
    );
  });
}
