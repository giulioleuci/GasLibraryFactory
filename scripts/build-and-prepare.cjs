...
if (mode === 'testoffline') {
  ...
  if (includeOfflineTests) {
    info(
      `Bundle tests: ${c.bold}${c.cyan}ENABLED${c.reset} — Jest tests will be compiled through webpack for offline verification`
    );
  }
} else {
  info(
...

{
  ...
  if (matches) {
    ...
    ok(
      `Replaced ${matches.length} instance(s) of Object.hasOwn() → Object.prototype.hasOwnProperty.call()`
    );
  } else {
...
if (warningCount > 0) {
  warn(`${warningCount} compatibility warning(s) detected — review above for details`);
}

step(5, TOTAL_STEPS, 'Configuring deployment files...');

const testFiles = fs.readdirSync(distDir).filter((f) => f.startsWith('TEST_') && f.endsWith('.gs'));
    ...
    try {
      ...
      if (sourceMap.sources && Array.isArray(sourceMap.sources)) {
        index.codeJs.sourceFiles = sourceMap.sources
          .map((s) =>
            s.replace(/^webpack:\/\/gaslibraryfactory-monorepo\//, '').replace(/^\.\//, '')
          )
          .filter(
            (s) =>
...
if (mode === 'testoffline') {
  ...
  try {
    execSync(
      'npx jest --config jest.config.cjs --reporters=summary --silent --no-stack-trace --no-cache',
      {
        stdio: 'inherit',
        cwd: rootDir
      }
  ...
  try {
    execSync(
      'npx jest --config jest.integration.config.cjs --reporters=summary --silent --no-stack-trace --no-cache',
      {
        stdio: 'inherit',
        cwd: rootDir
      }
  ...
  try {
    execSync(
      'npx jest --config jest.offline.config.cjs --reporters=summary --silent --no-stack-trace --no-cache',
      {
        stdio: 'inherit',
        cwd: rootDir
      }
...
if (includeOfflineTests) {
  ...
  const testWebpackCmd =
    'npx webpack --config webpack.offline-tests.config.cjs --mode production --env readable';
    ...
    if (fs.existsSync(testBundlePath)) {
      ...
      if (fs.existsSync(manifestPath)) {
        try {
          ...
          ok(
            `Test files compiled: ${manifest.totalTestFiles} across ${manifest.libraryCount} libraries`
          );
          for (const [libName, libInfo] of Object.entries(manifest.libraries)) {
            info(
              `  ${libName}: ${libInfo.testFileCount} test file(s) → ${libInfo.runnerFunction}()`
            );
          }
        } catch (_) {}
      }
...
if (mode === 'testoffline') {
  heading(
    offlineTestsPassed
      ? 'Build & Test Complete — All Tests Passed'
      : 'Build Complete — Tests Failed'
  );
} else {
...
if (mode === 'testoffline') {
  if (offlineTestsPassed) {
    ...
    if (offlineTestBundleBuilt) {
      info(
        `${c.green}Offline test bundle compiled successfully (webpack verification passed).${c.reset}`
      );
    }
