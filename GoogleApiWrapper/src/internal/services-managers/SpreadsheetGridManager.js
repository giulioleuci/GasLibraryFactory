...
export class SpreadsheetGridManager {
  ...
  formatRanges(spreadsheetId, formatRequests, options = {}) {
    ...
    if (this.facade._isDryRun(options)) {
      this._logger.info(
        `[DRY-RUN] Would format ${requestsArray.length} range(s) in spreadsheet ${spreadsheetId}`
      );
      return {
        updatedSpreadsheet: spreadsheetId,
        replies: requestsArray.map(() => ({})),
        dryRun: true
      };
    }
    ...
    const requests = requestsArray.map((req) => ({
      repeatCell: {
        range: this.facade._parseRangeToGridRange(req.range, sheets),
  ...
  setColumnWidths(spreadsheetId, widthRequests, _options = {}) {
    ...
    const requests = requestsArray.map((req) => ({
      updateDimensionProperties: {
        range: {
          sheetId: sheetMap[req.sheetName],
          dimension: 'COLUMNS',
          startIndex: req.startColumn,
          endIndex: req.endColumn
        },
        properties: { pixelSize: req.width },
        fields: 'pixelSize'
      }
    }));
  ...
  createSheets(spreadsheetId, sheetRequests, options = {}) {
    ...
    if (this.facade._isDryRun(options)) {
      this._logger.info(
        `[DRY-RUN] Would create ${requestsArray.length} sheet(s) in spreadsheet ${spreadsheetId}`
      );
      return {
        spreadsheetId,
        replies: requestsArray.map((req, i) => ({
          addSheet: {
            properties: { sheetId: `dry-run-sheet-${i}`, title: req.title, index: req.index || i }
          }
        })),
        dryRun: true
      };
    ...
    const requests = requestsArray.map((req) => ({
      addSheet: { properties: { title: req.title, index: req.index } }
    }));
    const result = this.facade._executeWithRetry(
      () => Sheets.Spreadsheets.batchUpdate({ requests }, spreadsheetId),
      { spreadsheetId },
  ...
  deleteSheets(spreadsheetId, sheetIds, options = {}) {
    ...
    if (this.facade._isDryRun(options)) {
      this._logger.info(
        `[DRY-RUN] Would delete ${idsArray.length} sheet(s) from spreadsheet ${spreadsheetId}`
      );
    ...
    sheets.forEach((sheet) => {
      sheetMap[sheet.properties.title] = sheet.properties.sheetId;
      sheetMap[sheet.properties.sheetId] = sheet.properties.sheetId;
    });
    const requests = idsArray.map((id) => ({
      deleteSheet: { sheetId: typeof id === 'string' ? sheetMap[id] : id }
    }));
    const result = this.facade._executeWithRetry(
      () => Sheets.Spreadsheets.batchUpdate({ requests }, spreadsheetId),
      { spreadsheetId },
  ...
  expandSheetGrid(spreadsheetId, sheetId, rowCount, columnCount) {
    ...
    this.facade._clearSheetMetadataCache(spreadsheetId);
    this._logger.info(
      `Expanded sheet grid (sheetId: ${sheetId}) to ${rowCount} rows × ${columnCount} columns`
    );
    return result;
  }
  ...
  protectRanges(spreadsheetId, protectionRequests, options = {}) {
    const requestsArray = Array.isArray(protectionRequests)
      ? protectionRequests
      : [protectionRequests];
    const globalEditors = options.globalEditors || {};
    const sheets = this.facade._getCachedSheetMetadata(spreadsheetId);
    this._logger.debug(
      `Protecting ${requestsArray.length} range(s) in spreadsheet: ${spreadsheetId}`
    );
    ...
    const requests = requestsArray.map((req) => {
      ...
      const editors = {
        ...
        domainUsersCanEdit:
          req.editors?.domainUsersCanEdit !== undefined
    ...
    const protectedRangeIds = result.replies.map(
      (r) => r.addProtectedRange.protectedRange.protectedRangeId
    );
