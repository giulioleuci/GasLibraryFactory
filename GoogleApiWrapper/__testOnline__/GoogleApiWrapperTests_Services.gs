...
function initGoogleApiWrapperTests_Services(NS) {
  ...
  runner.register(`${NS}/PropertiesService/UserAndDocument`, () => {
    ...
    try {
      ...
      SmartAssert.equals(propsService.getDocumentProperty(key), val, 'Should get document property');
  ...
  runner.register(`${NS}/TriggerService/Lifecycle`, () => {
    ...
    // 2. Check existence
    SmartAssert.isTrue(triggerService.triggerExistsForFunction(functionName), 'Trigger should exist');
    
    // 3. Delete by ID
    triggerService.deleteTriggerById(triggerId);
    SmartAssert.isFalse(triggerService.triggerExistsForFunction(functionName), 'Trigger should be deleted');
    
    // 4. Create daily trigger
    triggerService.createDailyTrigger(functionName, 9);
    SmartAssert.isTrue(triggerService.triggerExistsForFunction(functionName), 'Daily trigger should exist');
    
    // 5. Delete all triggers for function
    triggerService.deleteTriggersByFunction(functionName);
    SmartAssert.isFalse(triggerService.triggerExistsForFunction(functionName), 'All triggers for function should be deleted');
  });
