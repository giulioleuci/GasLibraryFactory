...
function initGoogleApiWrapperTests_Sheets(NS) {
  runner.register(`${NS}/SpreadsheetService/SheetManagement`, () => {
    ...
    const sheetService = new SpreadsheetService(
      deps.logger,
      deps.cache,
      deps.utils,
    ...
    sheetService.createSheets(
      ssId,
      sheetNames.map((name) => ({ title: name }))
    );
  ...
  runner.register(`${NS}/SpreadsheetService/ValidationAndFormatting`, () => {
    ...
    SmartAssert.equals(
      sheet.getConditionalFormatRules().length,
      1,
      'Conditional format rule should be added'
    );
  });
