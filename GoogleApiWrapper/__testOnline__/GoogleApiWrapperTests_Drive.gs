...
function initGoogleApiWrapperTests_Drive(NS) {
  runner.register(`${NS}/DriveService/FileLifecycle`, () => {
    ...
    SmartAssert.equals(
      DriveApp.getFileById(copyId).getName(),
      copyName,
  ...
  runner.register(`${NS}/DriveService/Permissions`, () => {
    ...
    const file = rootFolder.createFile(
      'PermissionTest_' + deps.utils.generateUuid().substring(0, 8),
      'Content'
    );
    const fileId = file.getId();
    ...
    SmartAssert.equals(
      userPerm.role,
      'owner',
      "Actually it might return owner if same user, but let's check what we got"
    );
  ...
  runner.register(`${NS}/DriveService/AdvancedFileCreation`, () => {
    ...
    SmartAssert.equals(
      ...
      'MimeType should be Google Docs'
    );
    ...
    SmartAssert.equals(
      ...
      'MimeType should be Google Sheets'
    );
  ...
  runner.register(`${NS}/DriveService/DetailedMetadata`, () => {
    ...
    const file = rootFolder.createFile(
      'MetadataTest_' + deps.utils.generateUuid().substring(0, 8),
      'Content'
    );
    const fileId = file.getId();

    // Get metadata via DriveService (Advanced API)
    const metadata = driveService.getFiles(fileId, {
      fields: 'id,name,size,owners,modifiedTime,capabilities'
    });
  ...
  runner.register(`${NS}/DriveService/FolderNavigation`, () => {
    ...
    SmartAssert.isTrue(
      fileNames.some((n) => n.includes('File1')),
      'Should find File1'
    );
    SmartAssert.isTrue(
      fileNames.some((n) => n.includes('SubSub')),
      'Should find SubSub folder'
    );
